...
public class MyList {
    ...
    void f2() throws Exception {
        ...
        Node curr = head;
        while (curr != null) {
            if (curr.getInfo().getDistrict().equals("Q2") || curr.getInfo().getDistrict().equals("Q9")) {
                curr.getInfo().setDistrict("TD");
                curr.getInfo().setPrice(curr.getInfo().getPrice() + 1);
            }
            curr = curr.next;
        }
        //------ End your code here-----------------------------------------------------------
    ...
    void f3() throws Exception {
        ...
        //------ Start your code here---------------------------------------------------------
        Node curr = head;
        int max = head.getInfo().getArea();
        while (curr != null) {
            if (curr.getInfo().getArea() > max) {
                max = curr.getInfo().getArea();
            }
            curr = curr.next;
        }
        Node curr1 = head;
        while (curr1 != null) {
            if (curr1.getInfo().getArea() == max) {
                clear();
                addLast(curr1.getInfo().getDistrict(), curr1.getInfo().getArea(), max);
                break;
            }
            curr1 = curr1.next;
        }
    ...
    void f4() throws Exception {
        ...
        do {
            ...
            while (curr.next != null) {
                Apartment currApart = curr.getInfo();
                Apartment nextApart = curr.next.getInfo();
                if (currApart.getPrice()< nextApart.getPrice()) {
                    Apartment temp = currApart;
                    curr.setInfo(nextApart);
                    curr.next.setInfo(temp);
                    swap = true;
                }
                curr = curr.next;
            }
